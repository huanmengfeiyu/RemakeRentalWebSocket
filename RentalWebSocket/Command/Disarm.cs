...
{
    public class Disarm : JsonSubCommand<RentalSession, RentalSocketList>
    {
        ...
        protected override void ExecuteJsonCommand(RentalSession session, RentalSocketList commandList)
        {
            try
            {
                ...
                bytelist.AddRange(ConvertHelpers.hexStrToByte("0x10"));

                operate.deviceList.Add(commandList);
                operate.Data = bytelist.ToArray();
                operate.comid = Name;
                RentalServer.oprateModelList.Enqueue(operate);
                Log.Warn(session.SessionID + ",执行了撤防");
                Log.Debug(session.SessionID + ",执行了撤防," + Newtonsoft.Json.JsonConvert.SerializeObject(commandList));
            }
            catch (Exception ex)
            {
